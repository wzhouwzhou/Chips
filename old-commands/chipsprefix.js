ex = {
  ...
  async func(msg, { member, guild, args, reply }) {
    ...
    if (action == 'status') {
      ...
      if (!customprefix[guild.id] || customprefix[guild.id] == prefix) return reply(`Set a custom prefix for me with \`\`${prefix}${ex.name} set\`\``);
      else return reply(`A prefix has already been set for this server! Check it with \`\`${prefix}${ex.name} (status)\`\``);
    }
  },
};
