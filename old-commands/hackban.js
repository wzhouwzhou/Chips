...
module.exports = {
  ...
  async func(msg, { send, reply, author, args, channel, guild }) {
    ...
    try {
      ...
      if (memberToUse == memberToUse.id) return reply("I can't let you ban yourself >.>");
    ...
    let reason;
    if (args[1]) {
      if (args[2] && args[2].toLowerCase() != 'dm') reason = _.drop(args).join(' ');
    ...
    collector.on('end', collected => {
      if (!confirmed) { return reply('Ban timed out'); } else {
        ...
        if (agreed) {
          ...
          client.users.fetch(memberToUse)
