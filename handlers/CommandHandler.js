module.exports = ({ Discord, client, permissions, Constants, _, Messager, moment }) => async(msg, prefix) => {
  ...
  let context = {
    ...
    bot: client,
    c: msg.channel,
    gMember: msg.guild ? msg.guild.member.bind(msg.guild) : $ => $,
  ...
  context.convertTime = (obj, i) => {
    if (obj instanceof Date) {
      if (isNaN(i)) throw new Error('Invalid number given for index');
      if (i >= context.times.length) throw new Error('Invalid index given');
  ...
  context.loadingBar = (msg_, options) => {
