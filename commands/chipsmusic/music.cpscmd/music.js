  ...
  async func(msg, { args, guild, client, member, channel, reply, send, prefix }) {
    ...
    if (args[0] === 'radio') {
      return send(new Discord.MessageEmbed().setTitle('Chips radio (Beta™)').setDescription([
        ...
        'Step 1: Have a voice channel with **`Chips Streams NCS`**, **`Chips Streams Monstercat`**, **`Chips Streams Listen.moe`**, **`Chips Streams Chillhop`**, or **`Chips Streams Classical`** in the name',
    ...
    let h;
    if (_handlers.has(guild.id)) { h = _handlers.get(guild.id); } else {
