...
module.exports = {
  ...
  func(msg, { reply, args }) {
    ...
    const pts = requestedUps != null && requestedUps[0] <= UPGRADEPTS &&
     requestedUps[0] >= 0 ? requestedUps[0] : UPGRADEPTS;
    for (let i = 0; i < pts; i++) {
      let ind = smasher && joinedcontent.toLowerCase().indexOf('auto') < 0 ?
        _.sample(PATHS2) : _.random(0, build.length - 1);
