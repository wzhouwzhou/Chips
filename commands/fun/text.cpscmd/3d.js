...
module.exports = {
  name: '3d',
  func(msg, { reply, send, prefix, content, guild, channel }) {
    ...
    if (split.length <= 0 || !split.every(w => w.length > 0 && !w.match(/\s+/g))) {
      ...
      if (!asciiCooldown.get((guild || channel).id)) {
        ...
        return split.forEach(word => (word && !word.match(/\s+/)) && asciify(word, { font: 'larry3d' }, (e, r) => {
    ...
    } else { return send('Too many words to 3dtext!').then(mm => mm.delete({ timeout: 3000 })); }
  },
};
