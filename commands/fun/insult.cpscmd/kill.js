		...
		if(!args) return msg.reply("Please mention someone to kill!");
			...
			if(!second) return msg.reply("Please mention a user to murder!");
				const killerMan = [
					...
					`${second} screams in terror as a giant creature with huge muscular arms grabs ${second}'s head; ${second}'s screams of terror are cut off as the creature tears off the head with a sickening crunching sound. ${second}'s spinal cord, which is still attached to the dismembered head, is used by the creature as a makeshift sword to slice a perfect asymmetrical line down ${second}'s body, causing organs to spill out as the two halves fall to their respective sides.`,

					`<@!${first}> grabs ${second}'s head and tears it off with superhuman speed and efficiency. Using ${second}'s head as a makeshift basketball, <@!${first}> expertly slam dunks it into the basketball hoop, much to the applause of the audience watching the gruesome scene.`,
					...
					`${second} steps on a land mine and is horrifically blown to multiple pieces as the device explodes, the ${second}'s entrails and body parts flying up and splattering all around as if someone had thrown a watermelon onto the ground from the top of a multiple story building.`,

					`${second} is killed instantly as the top half of his head is blown off by a Red Army sniper armed with a Mosin Nagant, ${second}'s brains splatter everywhere in a horrific fashion`
				];
