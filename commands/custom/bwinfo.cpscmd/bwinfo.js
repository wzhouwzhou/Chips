...
Several color roles exist, do \-roles\` in a bot channel and flip through the pages to choose one to ask staff for!`,
  ...
  desc_bwselfroles = `Selfroles can be assigned by using @Nadeko#6685 's .iam and .iamnot commands! To give yourself a selfrole just type \`.iam [Selfrole name]\` (without the []), to remove it do \`.aimnot\`
  Examples:
  \`.iam Female\`
A list of selfroles can be fetched by going to the bot channels and doing \`.lsar\`
To gain access to the NSFW channels use the \`-lewdme\` command! To remove yourself use \`-unlewdme\``,

  embed_bwinvite = new Discord.MessageEmbed()
    .setTitle(title_bwinvite)
    .setDescription(desc_bwinvite)
    .setColor('010000'),
  embed_selfroles = new Discord.MessageEmbed()
    .setTitle(title_bwselfroles)
    .setDescription(desc_bwselfroles)
    .setColor('B3Bf89'),
