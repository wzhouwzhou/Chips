...
ex = {
  ...
  async func(msg, { member, guild, args, reply}) {
    ...
    if(action=="status"){
      ...
      if((!client.customprefix[guild.id])||client.customprefix[guild.id]==prefix)
        return reply(`Set a custom prefix for me with \`\`${prefix}chipsprefix set\`\``);
      else
        return reply(`A prefix has already been set for this server! Check it with __${_.escapeRegExp(client.customprefix[guild.id])}chipsprefix status__`);
    }
  }
};
