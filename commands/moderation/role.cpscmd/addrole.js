  ...
  async func(msg, { send, guild, author, args, content, member, Discord }) {
    if (author.id === '205608598233939970') {
      ...
      if (args[0].match(/^[^]*<@!?(\d+)>[^]*$/) && args[1]) {
        ...
        return send(new Discord.MessageEmbed().setColor(member.displayColor).setDescription(`**Succesfully gave** <@&${targetRoleSend.id}> || ${targetRole} **to <@${targetUser.id}> || ${targetUser.id}`));
      }
    }
  },
};
