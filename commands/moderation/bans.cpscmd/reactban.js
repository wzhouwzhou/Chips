...
To stop this type \`\`${prefix}\`\`reactban ${timers.length-1}`);
      ...
      try{ //get mention:
        ...
        if(member.id == memberToUse.id)
          return reply("Why would you want to react ban yourself <.<");
        ...
        if(queryChannel!=channel.id){
          if(client.channels.get(queryChannel).guild.id==guild.id)
            return reply(`The specified reaction-ban id is for channel <#${queryChannel}>`);
          else
            return reply(`The specified reaction-ban id is for a channel unavailable to you!`);
        }else{
