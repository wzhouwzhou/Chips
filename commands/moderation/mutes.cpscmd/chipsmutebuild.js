...
module.exports = {
  ...
  async func(msg, { send, guild, reply, author }) {
    ...
    if (list.length > 1) return await reply(`There are too many Chips mute roles to use this command!`);


    if (list.length >= 1) return await reply(`There's already a mute role!`);


    else if (list.length < 1) send('Creating Mute Role...').then(m => setTimeout(() => m.delete({ timeout: 800 })), 5000);
