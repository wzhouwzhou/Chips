...
const Song = class Song {
    ...
    return new Promise((res, rej) => {
      ytdl.getInfo(this.url).then(info => {
        ...
        if (this._info.length_seconds) {
          ...
          for (const i in lengths) lengths[i] += keys[i].substring(0, 1);
          this._length = _.drop(lengths).reverse().splice(2)
            .join(' ');
  ...
  get stream() {
    if (this.livestream && this.livestream === '1') return ytdl(this.url, { quality: [95, 94, 93, 92, 91] });
