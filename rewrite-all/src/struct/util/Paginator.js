...
const Paginator = class Paginator {
  ...
  updateView() {
    return new Promise(async(res, rej) => {
      ...
      try {
        ...
        } else if (this.replying) { this.sentMsg = await this.sentMsg.edit(this._msg.author + this.currentText, { embed: this.embed }); } else { this.sentMsg = await this.sentMsg.edit(this.currentText, { embed: this.embed }); }
      } catch (err) {
        rej(err);
      }
  ...
  toggleLock(setting, requester, r) {
    if (!this.lockToggle) return this._msg.channel.send('The paginator controls may not be locked or unlocked!').then(mm => mm.delete({ timeout: 3000 }));

    if (setting) {
      if (this.locked) { return this._msg.channel.send('The paginator controls are already locked!').then(mm => mm.delete({ timeout: 3000 })); } else if (this._msg.author.id === requester.id) {
        this.locked = true;
      ...
      } else { return this._msg.channel.send(`${requester}, you cannot lock the paginator controls!`).then(mm => mm.delete({ timeout: 3000 })); }
    }

    if (!this.locked) { return this._msg.channel.send('The paginator controls are already unlocked!').then(mm => mm.delete({ timeout: 3000 })); } else if (this._msg.author.id === requester.id) {
      this.locked = false;
      this.sentMsg.react('🔒');
      r.remove('296855425255473154');
      return this._msg.reply('Everyone can operate the paginator controls now!').then(mm => mm.delete({ timeout: 3000 }));
    } else { return this._msg.channel.send(`${requester}, you cannot unlock the paginator controls!`).then(mm => mm.delete({ timeout: 3000 })); }
  }
  ...
  validateUpdatePage(direction) {
    ...
    } else if (typeof direction === 'number') { if (direction > -1 & direction < this.pages.length && direction !== this.currentPage) return true; }
    return false;
  }
